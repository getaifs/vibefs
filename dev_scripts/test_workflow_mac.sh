#!/bin/bash
# VibeFS End-to-End Workflow Test for macOS
...
cleanup() {
    ...
    if [ -d "$TEST_DIR" ]; then
        ...
        if mount | grep -q "$TEST_DIR"; then
            ...
            umount -f "${TEST_DIR}/.vibe/mounts"/* 2>/dev/null || true
        fi
...
MOUNT_POINT=$(find .vibe/mounts -name "$AGENT_ID*" -type d 2>/dev/null | head -n1)
if [ -z "$MOUNT_POINT" ]; then
    echo_error "Failed: mount point not found"
    exit 1
fi
...
# Test: vibe commit
echo_step "Test 7: vibe commit"
CURRENT_HEAD=$(git rev-parse HEAD)
"$VIBE_BIN" commit "$AGENT_ID" || {
    echo_error "Failed: vibe commit failed"
    kill $SPAWN_PID 2>/dev/null || true
    exit 1
}
...
if [ "$CURRENT_HEAD" = "$NEW_HEAD" ]; then
    echo_error "Failed: HEAD did not move after vibe commit"
    exit 1
fi
...
echo_success "vibe commit completed successfully"
